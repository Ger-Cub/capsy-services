    ...
    fetch(`${getAppointmentsUrl()}/?limit=10&future_only=true&mine_only=true`, { headers })
      ...
      .then((data: any[]) => {
        ...
        const upcoming = data
          ...
          .slice(0, 3)
          .map((a) => ({ id: a.data.id, name: a.data.name, start: a.data.start, appointment_status: a.data.appointment_status, location: a.data.location, user_id: a.data.user_id }));
        ...
        {!loading && appointments.length > 0 && (
          <div className="space-y-2">
            {appointments.map((a) => {
              ...
              return (
                <div key={a.id} className="flex items-center gap-3 p-2.5 rounded-xl hover:bg-brand-wellbeing/5 transition-colors">
                  <div className={`p-2 rounded-lg shrink-0 ${isBooked ? 'bg-emerald-50' : 'bg-brand-wellbeing/10'}`}>
