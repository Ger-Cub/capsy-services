...
export default function OdooDashboard() {
        ...
        return (
            <div className="min-h-screen bg-slate-950 flex items-center justify-center p-4">
                <div className="bg-white/5 backdrop-blur-xl border border-white/10 p-8 rounded-3xl w-full max-w-md shadow-2xl">
                    <h2 className="text-3xl font-black text-white mb-2">Connecter Odoo</h2>
                    <p className="text-slate-400 mb-8 text-sm">Entrez vos identifiants Odoo (essaiek3) pour accéder au pont Capsy.</p>

                    <form onSubmit={handleLogin} className="space-y-4">
                        <div>
                            <label className="block text-xs uppercase tracking-widest text-slate-500 font-bold mb-2">URL Odoo</label>
                            <input name="url" defaultValue="https://essaiek3.odoo.com" className="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-3 text-white outline-none focus:border-brand-wellbeing transition-colors" />
                        </div>
                        <div>
                            <label className="block text-xs uppercase tracking-widest text-slate-500 font-bold mb-2">Base de données</label>
                            <input name="db" defaultValue="essaiek3" className="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-3 text-white outline-none focus:border-brand-wellbeing transition-colors" />
                        </div>
                        <div>
                            <label className="block text-xs uppercase tracking-widest text-slate-500 font-bold mb-2">Email / Username</label>
                            <input name="username" defaultValue="bahavukevin@gmail.com" className="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-3 text-white outline-none focus:border-brand-wellbeing transition-colors" />
                        </div>
                        <div>
                            <label className="block text-xs uppercase tracking-widest text-slate-500 font-bold mb-2">Mot de passe / Token</label>
                            <input name="password" type="password" placeholder="••••••••" className="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-3 text-white outline-none focus:border-brand-wellbeing transition-colors" />
                        </div>
                        <button type="submit" className="w-full bg-gradient-to-r from-brand-wellbeing to-brand-green-fresh text-white font-bold py-4 rounded-xl shadow-lg hover:shadow-brand-wellbeing/20 transition-all active:scale-95">
    ...
    return (
        <div className="min-h-screen bg-slate-950 text-white font-sans overflow-x-hidden">
            {/* Header */}
            <header className="px-8 py-6 flex justify-between items-center border-bottom border-white/5 bg-slate-950/80 backdrop-blur-md sticky top-0 z-50">
                <div className="flex items-center gap-4">
                    <button
                        onClick={() => setCurrentApp(null)}
                        className={`p-2 rounded-lg hover:bg-white/10 transition-colors ${currentApp ? 'opacity-100' : 'opacity-0 pointer-events-none'}`}
                    >
                        ←
                    </button>
                ...
                <div className="flex items-center gap-4 bg-white/5 px-4 py-2 rounded-full border border-white/10">
                    <div className="w-8 h-8 rounded-full bg-brand-wellbeing flex items-center justify-center text-xs font-bold">BK</div>
                    <span className="text-sm font-medium">{creds?.username}</span>
                    <button onClick={() => { localStorage.removeItem('odoo_creds'); setShowLogin(true); }} className="text-slate-500 hover:text-white text-xs underline">Échanger</button>
                </div>
            </header>
                        ...
                        <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-6">
                            {APPS.map(app => (
                                <button
                                    ...
                                    className="group flex flex-col items-center gap-4 p-6 bg-white/5 border border-white/5 rounded-3xl hover:bg-white/10 hover:border-brand-wellbeing/30 transition-all hover:-translate-y-2"
                                >
                                    <div className={`w-20 h-20 ${app.color} rounded-2xl flex items-center justify-center text-3xl shadow-xl group-hover:scale-110 transition-transform`}>
                                        {app.icon}
                                    </div>
